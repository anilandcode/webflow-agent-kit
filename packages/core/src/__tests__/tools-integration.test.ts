...
describe('all tool groups — integration smoke', () => {
  ...
  function mk(overrides?: Record<string, any>) {
    const defaults = {
      sites: {
        ...
        redirects: { list: vi.fn().mockResolvedValue({ redirects: [] }), create: vi.fn(), delete: vi.fn() },
        scripts: { getCustomCode: vi.fn().mockResolvedValue({ scripts: [] }), upsertCustomCode: vi.fn(), deleteCustomCode: vi.fn() },
      ...
      assets: { list: vi.fn().mockResolvedValue({ assets: [] }), create: vi.fn(), get: vi.fn(), delete: vi.fn(), listFolders: vi.fn() },
      forms: { list: vi.fn().mockResolvedValue({ forms: [] }), get: vi.fn(), listSubmissions: vi.fn(), getSubmission: vi.fn(), updateSubmission: vi.fn() },
      webhooks: { list: vi.fn().mockResolvedValue([]), create: vi.fn(), get: vi.fn(), delete: vi.fn() },
      products: { list: vi.fn().mockResolvedValue({}), get: vi.fn(), create: vi.fn(), update: vi.fn() },
      orders: { list: vi.fn().mockResolvedValue({ orders: [] }), get: vi.fn(), update: vi.fn(), updateFulfill: vi.fn(), updateUnfulfill: vi.fn() },
      inventory: { list: vi.fn().mockResolvedValue({ quantity: 5 }), update: vi.fn() },
      ecommerce: { getSettings: vi.fn().mockResolvedValue({}) },
      components: { list: vi.fn().mockResolvedValue({ components: [] }), getContent: vi.fn(), updateContent: vi.fn(), getProperties: vi.fn(), updateProperties: vi.fn() },
  ...
  it('orders list works', async () => {
    const c = mk();
    const r = await createListOrdersTool(c).execute({ siteId: 's1', status: 'all', limit: 50, offset: 0 });
    expect(r).toBeDefined();
  });
  ...
  it('audit logs list works', async () => {
    const c = mk();
    const r = await createListAuditLogsTool(c).execute({ workspaceId: 'ws1', limit: 50, offset: 0 });
    expect(r).toBeDefined();
  });
